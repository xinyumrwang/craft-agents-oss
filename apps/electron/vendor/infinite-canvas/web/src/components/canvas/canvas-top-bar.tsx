...
import { BookOpen, Download, Keyboard, PanelLeftClose, PanelLeftOpen, Plus, Puzzle, Redo2, Settings, Settings2, Trash2, Undo2, Upload } from "lucide-react";
...
export function CanvasTopBar({
    ...
    return (
        <>
            <div
                className="pointer-events-none absolute right-0 top-0 z-[70] flex h-16 items-center justify-between pl-1 pr-4 transition-[left] duration-200"
                style={{ left: sidePanelOpen ? sidePanelWidth + 1 : 0 }}
            >
                ...
                <Dropdown
                    ...
                    menu={{
                        items: [
                            ...
                            { key: "docs", icon: <BookOpen className="size-4" />, label: t("topNav.docs"), onClick: () => window.open(DOCS_URL, "_blank", "noopener,noreferrer") },
                            { key: "config", icon: <Settings2 className="size-4" />, label: t("navigation.config"), onClick: () => openConfigDialog(false) },
                            { key: "shortcuts", icon: <Keyboard className="size-4" />, label: t("topNav.shortcuts"), onClick: () => setShortcutsOpen(true) },
                            { type: "divider" },
                ...
                >
                    <button
                        type="button"
                        className="pointer-events-auto grid size-8 place-items-center rounded-lg bg-transparent transition hover:bg-black/5 dark:hover:bg-white/10"
                        style={{ color: theme.node.text }}
                        aria-label="画布设置"
                        title="画布设置"
                    >
            ...
            <span className="flex min-w-0 flex-wrap items-center gap-1.5">
                {keys.map((key, index) => (
                    <span key={`${key}-${index}`} className="flex items-center gap-1.5">
                        ...
                        <kbd className="min-w-9 rounded-md border px-2.5 py-1.5 text-center text-xs font-medium leading-none" style={{ borderColor: "rgba(120,113,108,.28)" }}>
                            {key}
                        </kbd>
                    </span>
                ))}
