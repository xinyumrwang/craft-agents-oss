...
export function CanvasConfigNodePanel({ node, isRunning, inputSummary, onConfigChange, onGenerate, onStop, onComposerToggle }: CanvasConfigNodePanelProps) {
    ...
    const modeAppearance = MODE_APPEARANCE[mode];
    const ModeIcon = modeAppearance.icon;
    ...
    return (
        <div
            className="flex h-full w-full cursor-move flex-col px-3 pb-3 pt-7 text-sm transition-shadow duration-200"
            style={{ color: theme.node.text, boxShadow: `inset 3px 0 0 ${modeAppearance.accent}` }}
            onWheel={(event) => event.stopPropagation()}
        >
            <div className="mb-2 flex items-center justify-between gap-3">
                <div className="flex shrink-0 items-center gap-1.5 text-sm font-semibold">
                    <span className="inline-flex size-6 items-center justify-center rounded-md" style={{ background: modeAppearance.tint, color: modeAppearance.accent }}>
                        <ModeIcon className="size-3.5" />
                    </span>
                    <span>{t(modeAppearance.labelKey)}</span>
                </div>
            ...
            <div className="mb-2 flex flex-wrap gap-1.5">
                ...
                {(mode === "image" || mode === "text" || mode === "video") ? (
            ...
            <Button
                ...
                style={!isRunning ? { background: modeAppearance.accent, borderColor: modeAppearance.accent } : undefined}
            >
...
const MODE_APPEARANCE = {
    image: { icon: ImageIcon, accent: "#3b82f6", tint: "rgba(59, 130, 246, 0.14)", labelKey: "canvas.configNode.image" },
    text: { icon: MessageSquare, accent: "#8b5cf6", tint: "rgba(139, 92, 246, 0.14)", labelKey: "canvas.configNode.text" },
    video: { icon: Video, accent: "#ec4899", tint: "rgba(236, 72, 153, 0.14)", labelKey: "canvas.configNode.video" },
    audio: { icon: Music2, accent: "#f59e0b", tint: "rgba(245, 158, 11, 0.14)", labelKey: "canvas.configNode.audio" },
} as const;
