...
模型默认值优先由 ERP 中央策略的可选字段 `default_model` 控制，且该值必须同时存在于 `models` 授权列表中。若 ERP 对有效账号返回空的 `models`，Jonwork 将服务器后台已就绪的默认连接及其默认模型作为该账号唯一模型；端点、协议和凭据始终留在服务器，不下发客户端。若 ERP 显式返回非空列表，仍以该列表作为账号级白名单。托管客户端只显示服务器选定的模型名称，不开放提供商、模型或推理强度配置。

模型权限与展示配置分层处理：ERP 的 `models` 始终是最终授权集合；客户端模型连接可以为集合内的同一模型 ID 定义显示名称和说明，选择器会采用这些展示字段。客户端定义不会扩大授权范围，未出现在 ERP `models` 中的模型不会显示，也不能提交执行。
